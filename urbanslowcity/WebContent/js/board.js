        ...
        switch (Common.int(tempArr[i]["attribute"])) {
            ...
            case	7	:	//주소
                if(Common.int(tempArr[i]["attribute_value"]) == 1) {
                    FormCheck.setCheck(formName, tempArr[i]["code"]+'_post1', necessaryFlag, tempArr[i]["name"], 'num', '1-3');
...
function res_filesize(elm){
	if(document.getElementById('filesize_status').value != ""){
		if(document.getElementById('filesize_status').value == "false"){
...
function photo_file2(mode, com_board_id,file_size)
{
	...
	form.action = '/chtml/board.php?template=bizdemo18406&com_board_basic=image_upload_mode&filemode=' + filemode + '&com_board_id=' + com_board_id+'&filesize='+photo_max_size;
	form.submit();
